...
void MPU9250_GetData(int16_t* AccData, int16_t* MagData, int16_t* GyroData, float *TempData)
{
	...
	*TempData = (float)((((((int16_t)_buffer[6]) << 8) | _buffer[7]) - RoomTemp_Offset) / Temp_Sensitivity + 21.0f); //according to the register map file 4.23, TEMP_degC = ((TEMP_OUT - RoomTemp_Offset)/ Temp_Sensitivity) + 21degC
	...
	//according to the datasheet 3.4.2, RoomTemp_Offset = 21, Sensitivity = 333.87
	 ...
	 acc_total_vector= sqrt((AccData[0]*AccData[0])+(AccData[1]*AccData[1])+(AccData[1]*AccData[1]));
	  //57.296 = 1/(3.142/180)
	 AccData[0]=asin((float)AccData[0]/acc_total_vector)* 57.296;
	 AccData[1]= asin((float)AccData[1]/acc_total_vector)* -57.296;

	 GyroData[0]+= GyroData[1]*sin(GyroData[2]*0.000001066);
	 GyroData[1]-= GyroData[0]*sin(GyroData[2]*0.000001066);
  //(pi/180)= dereceyi radyana çevirme (0.000611/(3.142/180)=0.000001066)
